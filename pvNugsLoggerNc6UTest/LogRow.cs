...
internal class LogRow : ILoggerServiceRow
{
    ...
    /// <summary>
    /// Returns a string representation of this log entry containing all fields.
    /// </summary>
    /// <returns>A formatted string containing all log entry data</returns>
    public override string ToString()
    {
        return $"[{Id}] {CreationDateUtc:yyyy-MM-dd HH:mm:ss.fff} [{Severity}] " +
               $"UserId: {UserId ?? "N/A"}, CompanyId: {CompanyId ?? "N/A"}, " +
               $"Topic: {Topic ?? "N/A"}, Machine: {MachineName}, " +
               $"Member: {MemberName}, File: {FilePath}:{LineNumber}, " +
               $"Message: {Message}";
    }
}